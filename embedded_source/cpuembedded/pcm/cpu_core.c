	...
	**************************************************************/
		...
		switch (ucCommand)
		{
			...
			**************************************************************/
				...
				command_byte_data();
				break;
			case CPU_COMMAND_HIGH_SPEED_CLK:
			/**************************************************************
			*
			* V1.2.0 Send data with High speed CCLK.
			*
			**************************************************************/
				
				command_high_speed_clk();
				break;
				
			case CPU_COMMAND_READ_REGISTER:
			/**************************************************************
			*
			* V1.2.0 Read and Compare one frame of bitstream data byte by byte.
			*
			**************************************************************/
				scRetCode = command_byte_verify_data();
				break;
			case CPU_COMMAND_VERIFY_REGISTER:
			/**************************************************************
			*
			* V1.2.0 Read and Compare one frame of bitstream data byte by byte with mask.
			*
			**************************************************************/
				scRetCode = command_byte_verify_data_with_mask();
				break;
...
{
	unsigned short usClockSize = 0x0000;
	unsigned char ucData = 0;
	/**************************************************************
	*
	* Get the byte of data and map to the data pins.
	*
	**************************************************************/
	ucData = get_byte();
	set_byte_data(ucData);
	/**************************************************************
	*
	* Get the number of the clocks(CCLK).
	*
	**************************************************************/
	
	usClockSize = get_number();
	for (; usClockSize > 0; usClockSize--)
	{
	/**************************************************************
	*
	* Drive CCLK low and write the data out.
...
signed char command_byte_verify_data()
{
	...
	**************************************************************/
	...
	/**************************************************************
	*
	* Iterate through the size of the data read and compare each byte 
	* to the port.
	*
	**************************************************************/
	if (compressed)
	{
		...
		**************************************************************/
		for (usDataSize = 0; usDataSize < size; usDataSize++)
		{
			...
			ucData = ByteData[usDataSize];
			cReadByte =(unsigned short) in_port();
			if (cReadByte != getUsRegister())
			{
				return CPU_VERIFICATION_FAILURE;
			}	
...
signed char command_byte_verify_data_with_mask()
{
	...
	signed char compression = 0;
	unsigned char *ByteData = 0;
	unsigned char *ByteMaskData = 0;
	unsigned short usDataSize = 0x0000;
	unsigned char ucData = 0;
	unsigned char cReadByte = 0;
	unsigned char cMaskByte = 0;
	unsigned char cCompareResult = 0;
	unsigned char ucTempData = 0x00;
	int ucByteIndex;
	...
	if (compressed)
	{
		...
		***************************************************************/
		...
		ByteMaskData =(unsigned char*)malloc((usDataSize + 1) * sizeof(unsigned char));
		if(!ByteMaskData)
			return CPU_OUT_MEMORY;
			...
			case 0xFF: 
				...
				for (index = 0; index < size; index++)
				{
					...
					for (loop_var1 = 0; loop_var1 < loop_var2; loop_var1++)
					{
						...
						ByteMaskData[index] |=((cDataByte << loop_var0++)&0x80) >> loop_var1;
					} 
				}     
				size = 0;
				break;
			...
			default:   
				...
				for (index = 0; index < compress; index++)
				{
					...
					for (loop_var0 = 0; loop_var0 < size*2/compress; loop_var0++)
					{
						...
						{
							if (index%2) 
								ByteMaskData[loop_var1/2] |= cDataByte & 0x0F;
							else 
								ByteMaskData[loop_var1/2] |= cDataByte >> 4;
						}
						else 
						{
							if (index%2) 
								...
								ByteMaskData[loop_var1/2] |= cDataByte & 0xF0;
						}
					}
				}
				size = 0;
		...
		**************************************************************/
		for (index = 0; index < size; index++)
		{
			...
			ucData = ByteData[index];
			cMaskByte = ByteMaskData[index];
			cReadByte =(unsigned short) in_port();
			cCompareResult = (ucData & cMaskByte)^(cReadByte & cMaskByte);
			if (cCompareResult) 
			{
				if(ByteData)
					free(ByteData);
	...
	else
	{
		...
		if(!ByteData)
			return CPU_OUT_MEMORY;
		for ( index = 0; index < usDataSize ; index++ ) {
			ByteData[index] = get_byte();
		}
		ByteMaskData =(unsigned char*)malloc((usDataSize + 1) * sizeof(unsigned char));
		if(!ByteMaskData)
			return CPU_OUT_MEMORY;
		for ( index = 0; index < usDataSize ; index++ ) {
			ByteMaskData[index] = get_byte();
		}
		for (index = 0; index < usDataSize; index++)
		{
