...
void ispVMComment();
short int ispVMLoop( unsigned short a_usLoopCount );
extern int ToggleTRST(int toggle);
extern int SetI2CStartCondition();
		...
		*************************************************************/
		...
		case I2C_LOOP:
			...
			*************************************************************/
			...
			printf( "LoopCount %d\n", g_usLCOUNTSize );
#endif
			siRetCode = ispVMLoop( ( unsigned short ) g_usLCOUNTSize );
			if ( siRetCode != 0 ) {
				return ( siRetCode );
			}			
			break;
...
short int ispVMRead( unsigned int a_uiDataSize )
{
	...
	if( a_uiDataSize == 32)
	{
		if(ReadBytesAndSendNACK(a_uiDataSize, (unsigned char *)ReadData, 1))
		{
			return(-1);
		}
	}
	...
	{
		if(ReadBytesAndSendNACK(a_uiDataSize, (unsigned char *)ReadData, 0))
		{
			return(-1);
		}
	}
	...
	for ( uiIndex = 0; uiIndex < a_uiDataSize; uiIndex++ ) { 
		if ( uiIndex % 8 == 0 ) {
			...
			cInDataByte = (unsigned char)ReadData[usBufferIndex++];
		}
...
short int ispVMSend( unsigned int a_uiDataSize )
{
	...
	unsigned char g_pucInData[1024] = {0};
	/*************************************************************
	*                                                            *
