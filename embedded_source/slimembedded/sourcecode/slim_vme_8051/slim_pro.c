...
unsigned char g_ucCompressCounter = 0; /*** used to indicate how many times 0xFF or 0x00 is repeated ***/
unsigned char g_iCompressKey = 0xFF; /*** used to indicate the compression key 0xFF or 0x00 ***/
	...
	{ 
		if (uiIndex % 8 == 0)
		{
			if ( g_usDataType & TDI_DATA ) {
				...
				*************************************************************/
				if ( g_ucCompressCounter ) {
					g_ucCompressCounter--;
					ucTDIByte = ( unsigned char ) g_iCompressKey;
				}
				else {
					...
					/*************************************************************
					*															 *
					* The g_iCompressKey is the compression key value use in the *
					* compression 0xFF or 0x00. Assume the key is 0xFF (default).*
					...
					*                                                            *
					*************************************************************/
					if ( ( g_usDataType & COMPRESS_FRAME ) && ( ucTDIByte == ( unsigned char ) g_iCompressKey ) ) {
			...
			*************************************************************/
				...
				{
					g_ucCompressCounter--;
					ucTDOByte =(unsigned char) g_iCompressKey;
				}
				else 
				{
					...
					*                                                            *
					* The g_iCompressKey is the compression key value use in the *
					* compression 0xFF or )x00. Assume the key is 0xFF (default).*
					* If the frame is compressed and the byte is 0xFF, then the  *
					* next couple bytes must be read to determine how many       *
					* repetitions of 0xFF are there.  That value will be stored  *
					* in the variable g_ucCompressCounter.                       *
					*                                                            *
					*************************************************************/
					if ((g_usDataType & COMPRESS_FRAME) &&(ucTDOByte ==(unsigned char) g_iCompressKey))
					{
						g_ucCompressCounter = GetByte(g_iMovingDataIndex++, 0);
						g_ucCompressCounter--;
					}
	...
	{ 
		...
		{ 
			...
			*************************************************************/
				...
				{
					g_ucCompressCounter--;
					ucCurByte =(unsigned char) g_iCompressKey;
				}
				else 
				{
					...
					*************************************************************/
					...
					if ((g_usDataType & COMPRESS_FRAME) &&(ucCurByte ==(unsigned char) g_iCompressKey))
					{
						g_ucCompressCounter = GetByte(g_iMovingDataIndex++, 0);
						g_ucCompressCounter--;
					}
