...
/***************************************************************
*
* Global variable to store the delay percentage.
*
***************************************************************/

unsigned short g_usDelayPercent = 0;
...
extern const unsigned char g_ucPinCE;
	...
	while ( ( cDataByte = GetByte() ) >= 0 ) { 
			...
			case TDI:
				...
				*****************************************************************************/
				...
				/****************************************************************************
				*
				* When converting VME to HEX. Must guarantee that g_usTDOSize is at least as
				* large as g_usTDISize. This is necessary for the XTDO support which repeatedly
				* copies over TDI buffer to the TDO buffer. If the TDO buffer is smaller than
				* TDI, it ends up copying data to undefined memory spaces.
				*
				*****************************************************************************/
		...
		default:   
			...
			for (index = 0; index < compress; index++) {
				...
				for (i = 0; i < size*2/compress; i++){
					j = (unsigned short)(index + i*compress);
...
void sclock()
{
	...
	if ( g_iFrequency >= 900 ) {
		IdleTime = 0;  /* No additional idle time if frequency is at least 900 KHz */
	}
	else {
		IdleTime = ( unsigned short ) ( 1000 / g_iFrequency + 1 ); /*delay the clock for 3K devices*/
	}

	IdleTime++;
	for ( usIdleIndex = 0; usIdleIndex < IdleTime; usIdleIndex++ ) {	/* delay the clock by loop*/
...
void ispVMStart()
{
	...
	writePort( g_ucPinCE, 0x01 );   /*turn the Lattice Cable on*/
...
#endif
	g_cCurrentJTAGState = RESET;
	ispVMStateMachine( RESET );   /*step devices to RESET state */
	ispVMDelay( 1000 );              /*wake up devices*/
	writePort( g_ucPinCE, 0x00 );  /*disable the Lattice Cable*/
}
...
signed char ispVMSend( unsigned short a_usiDataSize )
{
	...
	/****************************************************************************
	*
	* If converting VME to HEX, hardware is not required.
	*
	*****************************************************************************/
